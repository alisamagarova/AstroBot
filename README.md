...
> и доставку сообщений через бота (уведомления, шэр PDF) — chart-эндпойнты остаются
> заглушками (`501`), так как карты строятся у клиента. Это важно для нагрузки:
> тяжёлые вычисления вынесены на устройства пользователей и не нагружают сервер.

### Реализованные услуги (фичи)

| Услуга | Где считается | Сохранение в БД |
|---|---|---|
| Натальная карта (своя) | браузер | профиль в `people` |
| Натальная карта для другого человека | браузер | **не сохраняется**; PDF уходит в Telegram (через бота, транзитом) |
| Синастрия | браузер | партнёры в `people` |
| Соляр | браузер | профиль + город проживания |
| Аспекты на месяц | браузер (курированные тексты `transits_*.json`) | отметки просмотра (`aspect_views`) |
| Жизненные вехи | браузер | — |
| Хорар (точечные вопросы) | браузер (`horar-engine.js`) | localStorage-кулдаун; **не сохраняется** |
| Уведомления (соляр/аспекты) | планировщик в боте | настройки + журнал (`notify_*`, `*_views`, `notifications_sent`) |
...
| Бот | `src/bot/bot.ts` | grammY: `/start` → кнопка-ссылка на Main App (`?startapp`), проверка профиля; режимы long polling / webhook |
| Планировщик | `src/bot/scheduler.ts` | Раз в 12 ч проверяет условия и шлёт уведомления о соляре/аспектах (антидубль через `notifications_sent`) |
...
| Роуты | `src/routes/*.ts` | `users`, `people`, `charts` (501-заглушки), `notifications`, `share` (PDF в Telegram), `admin` (только dev) |
| Доступ к БД | `src/db/queries/*.ts` | Параметризованные запросы, бизнес-правило блокировки даты, уведомления |
| Конфиг | `src/config.ts` | Типизированные env-переменные, `isDev`, `webhookUrl` |
...
| `onboarding_draft` | `JSONB` | NO | `'{}'` | — | **Не используется** (наследие онбординга-диалога в боте; можно удалить) |
...
| `kind` | `TEXT` IN (`solar`,`aspects`) | Тип уведомления |
| `ref` | `TEXT` | Год соляра (`'2027'`) или месяц аспектов (`'2026-07'`) |
...
> монетизация), но текущий код их **не читает и не пишет** — карты считаются в
> браузере, платежи не реализованы. Их триггеры инвалидации кэша по факту no-op.
> К удалению **не обязательны**; держим под будущую серверную реализацию.
>
...
│   ├── bot/
│   │   ├── bot.ts            # grammY: /start, кнопка app, webhook/long-polling
│   │   ├── scheduler.ts      # планировщик уведомлений (соляр/аспекты)
...
(в корне репозитория: render.yaml — blueprint для Render; db/scripts/003_notifications.sql — миграция уведомлений)
```
...
| Метод | Путь | Описание |
|---|---|---|
| `GET` | `/notifications` | Настройки уведомлений (`notify_solar/aspects/viewed`) |
...
- **Активно используются:** `users` (вкл. `notify_*`, `onboarding_*`), `people`,
  `legal_consents`, `solar_views`, `aspect_views`, `notifications_sent`.
- **Поддержано, но не заполняется:** `people.name_en` (фронт пока не шлёт транслитерацию).
- **Мёртвое:** `users.onboarding_draft` (JSONB) — наследие бота-диалога, можно удалить.
- **Зарезервировано (код не трогает):** `natal_charts`, `solar_charts`, `synastry_reports`,
  `aspect_snapshots`, `milestone_reports`, `horar_charts`, `payments` — под будущий
  серверный кэш и платежи.
